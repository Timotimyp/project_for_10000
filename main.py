...
def func(message):
    if message.text == "Изменить Категорию":
        ...
        q = requests.get('http://127.0.0.1:5000/api/sorted_keys').json()['q']
        if q:
            for i in q:
...
def self_delivery(message):
    try:
        if message.text != None:
            try:
                w = int(message.text)
                r = requests.post('http://127.0.0.1:5000/api/self_delivery_post', json={'cost': w}).json()
...
def delivery_delivery(message):
    try:
        if message.text != None:
            try:
                int(message.text)
                r = requests.post('http://127.0.0.1:5000/api/delivery_delivery_post',
...
def porog_delivery(message):
    try:
        if message.text != None:
            try:
                int(message.text)
                r = requests.post('http://127.0.0.1:5000/api/porog_delivery_post',
                                  json={'cost': int(message.text)}).json()
                if "success" in dict(r).keys():
                    bot.send_message(message.chat.id, "Всё успешно выполнено", reply_markup=markup)
            except ValueError:
...
def rr(message):
    try:
        if message.text != None:
            ...
            r = requests.post('http://127.0.0.1:5000/api/category_new_post', json={'cat': message.text,
...
def rr2(message, e):
    try:
        if message.text != None:
            ...
            r = requests.post('http://127.0.0.1:5000/api/category_correct_post',
...
def rr3(message, e, name, photo, description):
    try:
        int(message.text)
        r = requests.post('http://127.0.0.1:5000/api/position_new_post',
...
def posit_first(message, category):
    fi2 = requests.get(f'http://127.0.0.1:5000/api/get_first_fi2/{category}').json()
    try:
...
def correct_pos(message, posit, cat_old):
    try:
        if message.text != None:
            ...
            r = requests.post('http://127.0.0.1:5000/api/position_correct_posit_name_post',
...
def correct_photo(message, posit, cat_old):
    if message.photo:
        ...
        with open("image.jpg", 'wb') as new_file:
            new_file.write(downloaded_file)
        encoded = base64.b64encode(open("image.jpg", "rb").read())
        r = requests.post('http://127.0.0.1:5000/api/position_correct_posit_photo_post',
...
def correct_description(message, posit, cat_old):
    try:
        if message.text != None:
            r = requests.post('http://127.0.0.1:5000/api/position_correct_posit_description_post',
...
def correct_cost(message, posit, cat_old):
    try:
        int(message.text)
        try:
            r = requests.post('http://127.0.0.1:5000/api/position_correct_posit_cost_post',
...
def inline(c):
    ...
    if c.data in q:
        ...
        try:
            ...
            r = requests.post('http://127.0.0.1:5000/api/delete_category_post', json={'cat_old': cat}).json()
            if "success" in dict(r).keys():
        ...
        try:
            if cat_code[c.data[8:]] in q:
                cat = c.data[8:]
                cat = cat_code[cat]
                fi2 = requests.get(f'http://127.0.0.1:5000/api/get_first_fi2/{cat}').json()['fi2']
        ...
        try:
            if cat_code[c.data.split('/')[0]] in q:
                ...
                fi = requests.get(f"http://127.0.0.1:5000/api/get_fi/{cat}").json()['fi']
                cat_old = fi
        ...
        try:
            fi = requests.get(f'http://127.0.0.1:5000/api/get_fi/{c.data[8:]}').json()['fi']
            fi2 = requests.get(f'http://127.0.0.1:5000/api/get_first_fi2/{c.data[8:]}').json()['fi2']
        ...
        try:
            r = requests.post('http://127.0.0.1:5000/api/0_4',
        ...
        try:
            ...
            s_name.pop(pos2)
            r = requests.post('http://127.0.0.1:5000/api/delete_position_post',
